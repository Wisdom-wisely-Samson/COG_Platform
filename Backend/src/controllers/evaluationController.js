...
// ── Department-specific AI report prompts (used when real API key is present) ─
const DEPT_SYSTEM = {
  "PR & Media": `You are a senior PR and communications expert generating a stakeholder report for Capital One Group, a leading Nigerian financial services company.

Produce a concise executive summary and actionable report for the submitted task and file(s), then assess the work across these 4 criteria (each scored 0–25):
...
4. Compliance & Accuracy — Does it avoid misleading claims and align with financial communications standards?`,
  ...
  Digital: `You are a digital marketing strategist generating a stakeholder report for Capital One Group, a leading Nigerian financial services company.

Summarise the submitted work, provide an executive overview, and produce an actionable report for the task and file(s). Then assess the work across these 4 criteria (each scored 0–25):
...
4. Call to Action — Is there a clear, compelling CTA that drives the desired user behaviour?`,
  ...
  Creative: `You are a creative director generating a stakeholder report for Capital One Group, a leading Nigerian financial services company. The brand colours are red (#FF6600) and navy (#0D2B4F).
...
4. Creative Concept — Is the idea original, relevant, and effective in communicating the intended message?`,
  ...
  "Admin / Ops": `You are a senior operations manager generating a stakeholder report for Capital One Group, a leading Nigerian financial services company.
...
const DEFAULT_SYSTEM = `You are an AI assistant producing a concise stakeholder report for Capital One Group, a leading Nigerian financial services company.

Summarise the submitted work, provide a clear executive overview, and deliver an actionable report that evaluates the submission across these 4 criteria (each scored 0–25):
...
CRITICAL REPORT RULES — YOU MUST FOLLOW THESE EXACTLY:
...
function analyseTask(task, deptName) {
  ...
  const isPressRelease = /press release|press-release|news release/i.test(
    lower,
  );
  const isPoster = /poster|banner|flyer|billboard/i.test(lower);
  const isSocialMedia =
    /social media|instagram|twitter|facebook|linkedin|tiktok|post/i.test(lower);
  const isVideo =
    /video|reel|clip|animation|motion/i.test(lower) ||
    files.some((f) => /\.(mp4|mov|webm|avi)$/i.test(f.originalName || ""));
  const isReport = /report|brief|memo|document|summary|minutes|proposal/i.test(
    lower,
  );
  const isCampaign = /campaign|promotion|launch|ad |advert|advertisement/i.test(
    lower,
  );
  const isEmail = /email|newsletter|mailer/i.test(lower);
  const isPresentation = /presentation|slide|deck|powerpoint|keynote/i.test(
    lower,
  );
  const isGraphic = /graphic|design|logo|artwork|visual|infographic/i.test(
    lower,
  );
  const isPhoto =
    /photo|photograph|image/i.test(lower) ||
    files.some((f) => /\.(jpg|jpeg|png|gif)$/i.test(f.originalName || ""));
...
function buildCriterionFeedback(criterionName, ctx, baseScore) {
  ...
  const fileRef = firstFile
    ? `"${firstFile}"`
    : `the submission for "${title}"`;
  const taskRef = `"${title}"`;
  const quality =
    baseScore >= 22 ? "strong" : baseScore >= 18 ? "adequate" : "limited";
  const impact =
    baseScore >= 22
      ? "effectively"
      : baseScore >= 18
  ...
  const feedbacks = {
    ...
    "Clarity & Structure": () => {
      if (isReport)
        return `The structure of ${fileRef} ${impact} aids navigation and comprehension — ${baseScore >= 20 ? "the use of clear headings, numbered sections, and logical information flow makes the document easy to navigate" : "the document would benefit from a clearer heading hierarchy and more consistent section formatting to improve readability"}. ${notes ? `Given the stated context "${notes.slice(0, 50)}…", the audience is likely ${baseScore >= 20 ? "well-served by the current structure" : "expecting a more structured layout with clearer signposting between sections"}` : `For ${taskRef}, adding a table of contents and numbered sections will significantly improve navigability for senior reviewers`}.`;
  ...
  return fn
    ? fn()
    : `${fileRef} was assessed against the ${criterionName} dimension for ${deptName || "Capital One Group"} standards — the submission ${impact} meets expectations for this criterion, with ${quality === "strong" ? "minimal" : quality === "adequate" ? "moderate" : "significant"} revision required to reach the optimal standard for ${taskRef}.`;
}
...
function buildStrengths(ctx, total, template) {
  ...
  if (hasFiles) {
    if (isVideo)
      specific.push(
        `The video format chosen for "${title}" is appropriate for digital distribution and demonstrates strong production awareness.`,
      );
    else if (isPoster)
      specific.push(
        `The poster/banner format of ${fileRef} is well-suited to the campaign objective and the file has been submitted in a workable format.`,
      );
    else if (isPressRelease)
      specific.push(
        `"${title}" follows the structural conventions of a press release, which will ease the editorial review process.`,
      );
    else if (isReport)
      specific.push(
        `"${title}" demonstrates sound document discipline — the structured format aids the review and approval workflow.`,
      );
    else if (isSocialMedia)
      specific.push(
        `The social media format of ${fileRef} reflects an understanding of platform content expectations.`,
      );
    else
      specific.push(
        `${fileRef} has been submitted in an appropriate file format, enabling direct review and feedback without format conversion.`,
      );
  }

  if (priority === "High")
    specific.push(
      `The high-priority classification of "${title}" has been acknowledged and the submission is timely relative to the expected delivery window.`,
    );
  if (notes)
    specific.push(
      `The accompanying task notes provide useful context that aids the reviewer's understanding of the intent behind "${title}".`,
    );
  if (total >= 75)
    specific.push(
      `The overall quality of "${title}" positions it as a ${deptName || "departmental"} reference piece that future submissions can benchmark against.`,
    );
...
function buildImprovements(ctx, total, template) {
  ...
  if (isPressRelease)
    specific.push(
      `Add a media contact block at the end of "${title}" including a named spokesperson, direct phone number, and email address — this is mandatory for all Capital One Group press releases.`,
    );
  if (isPoster)
    specific.push(
      `Ensure ${fileRef} is exported at 300 DPI with 3mm bleed on all sides for any physical print production; supply an RGB version separately for digital placements.`,
    );
  if (isSocialMedia)
    specific.push(
      `Strengthen the CTA in "${title}" with a platform-specific directive (e.g., "Swipe up", "Link in bio", "Comment below") that ties directly to the campaign's conversion goal.`,
    );
  if (isVideo)
    specific.push(
      `Add burnt-in captions to ${fileRef} — at least 85% of social video is consumed without sound, making captions essential for message delivery and accessibility compliance.`,
    );
  if (isReport)
    specific.push(
      `Prepend an executive summary (maximum one page) to "${title}" — senior leadership reviewers need to extract key decisions and recommendations without reading the full document.`,
    );
  if (isEmail)
    specific.push(
      `Test "${title}" across at least three email clients (Gmail, Outlook, Apple Mail) and two screen sizes before sending — rendering inconsistencies in Outlook are the most common cause of failed email campaigns.`,
    );
  if (isCampaign)
    specific.push(
      `Develop a measurement framework for "${title}" before launch — define KPIs (reach, engagement rate, click-through, conversion) and set up tracking so performance can be reported within 48 hours of going live.`,
    );

  if (!specific.length && hasFiles)
    specific.push(
      `Include a version number, author name, and submission date in ${fileRef} to facilitate version control and audit trail requirements.`,
    );
  if (!notes)
    specific.push(
      `Add a notes field to "${title}" describing the intended audience, distribution channel, and campaign objective — this context will significantly improve the quality of future AI reports.`,
    );

  const templateImprovements = template.improvements.map((s) =>
    s.replace(
      /\.$/,
      ` — apply this specifically to the "${title}" submission.`,
    ),
  );
...
/**
 * Build a task-specific mock report with unique, contextual feedback
...
function buildMockEvaluation(task, deptName) {
  ...
  const criteria = template.criteria.map((c, i) => {
    ...
    const score = Math.min(
      25,
      Math.max(10, c.base + variation + priorityBonus + (hasFiles ? 1 : 0)),
    );
  ...
  try {
    ...
    if (!apiKey || apiKey === "your_anthropic_api_key_here") {
      return res.status(500).json({
        message:
          "AI report generation is unavailable. Configure a valid ANTHROPIC_API_KEY in Backend/.env and restart the server.",
      });
    }

    // ── Live mode: call Claude ────────────────────────────────────────────────
    const systemText =
      (DEPT_SYSTEM[deptName] || DEFAULT_SYSTEM) + JSON_INSTRUCTION;
    ...
    for (const att of attachments) {
      if (IMAGE_TYPES.has(att.mimetype) && att.size <= MAX_IMAGE_BYTES) {
        ...
        if (fs.existsSync(filePath)) {
          ...
          content.push({
            type: "text",
            text: `[Image above is the file named "${att.originalName}" — evaluate what you can observe in this image directly.]`,
          });
          imageCount++;
        }
      } else {
    ...
    if (status === 401) {
      return res
        ...
        .json({
          message:
            "Invalid Anthropic API key. Open Backend/.env, replace ANTHROPIC_API_KEY with your real key from console.anthropic.com, then restart the server.",
        });
    }
    if (status === 403) {
      return res
        ...
        .json({
          message:
            "Your Anthropic API key does not have permission to use this model. Check your plan at console.anthropic.com.",
        });
    }
    if (status === 429) {
      return res
